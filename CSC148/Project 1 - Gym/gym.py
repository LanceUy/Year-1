...
class Instructor:
    ...
    def can_teach(self, workout_class: WorkoutClass) -> bool:
        ...
        for cert in workout_class.get_required_certificates():
            if cert not in self._certificates:
                return False
        return True
...
class Gym:
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        _schedule: Dict[datetime,
                    Dict[str, Tuple[Instructor, WorkoutClass, List[str]]]]
        """
        # Testing if instructor has certs to teach
        if not self._instructors[instr_id].can_teach(
                self._workouts[workout_name]):
            return False

        # Check if room is available
        if time_point in self._schedule and room_name in \
                self._schedule[time_point]:
            return False

        if time_point in self._schedule:
            for item in self._schedule[time_point]:
                if self._schedule[time_point][item][0] == \
                        self._instructors[instr_id]:
                    return False

        # Final step update schedule
        # need to append to existing time point not replace
        # if else if not schedule already then:
        if time_point not in self._schedule and room_name in self._rooms \
                and instr_id in self._instructors and workout_name \
                in self._workouts:
            self._schedule[time_point] = {room_name:
                                              (self._instructors[instr_id],
                                               ...
                                               [])}
            return True
        elif time_point in self._schedule and room_name in self._rooms \
                and instr_id in self._instructors and workout_name \
                in self._workouts:
            self._schedule[time_point][room_name] = (
                (self._instructors[instr_id],
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        False
        """
        # Check if client is already in a class at the same time
        check = True
        if time_point in self._schedule:
            for items in self._schedule[time_point]:
                if client in self._schedule[time_point][items][2]:
                    check = False

        if time_point in self._schedule:
            for item in self._schedule[time_point]:
                if self._schedule[time_point][item][1].get_name() == \
                        workout_name and check and \
                        len(self._schedule[time_point][item][2]) < \
                        self._rooms[item]:
                    self._schedule[time_point][item][2].append(client)
                    return True
        return False
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        lst = []
        if time_point in self._schedule:
            for key, value in self._schedule[time_point].items():
                lst.append((value[0].name, value[1].get_name(), key))
        return lst
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        # time1 <= date <= time2
        lst = {}
        # need to populate lst with ids and 0
        for ins_id in self._instructors:
            lst[ins_id] = 0

        for date, value in self._schedule.items():
            for room in value:
                if time1 <= date <= time2 and value[room][0].get_id() in lst:
                    lst[value[room][0].get_id()] += 1
        return lst
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        lst = []
        for key, value in self.instructor_hours(time1, time2).items():
            num = BONUS_RATE * self._instructors[key].get_num_certificates()
            lst.append((key, self._instructors[key].name, value,
                        ((num + base_rate) * value)))
        lst.sort()
        return lst
