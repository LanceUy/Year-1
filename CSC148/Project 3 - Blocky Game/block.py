...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        if self.children:
            ...
            for i in range(len(self.children)):
                child_posx = position[0]
                child_posy = position[1]
                if i in (0, 3):
                    child_posx = position[0] + size
                if i in (2, 3):
                    child_posy = position[1] + size
                self.children[i].position = (child_posx, child_posy)
        ...
        if self.smashable():
            for _ in range(4):
                self.children.append(
                    self._smash_helper(self.level + 1, self.max_depth,
                                       self._child_size()))
    ...
    def _smash_helper(self, level: int, maxi: int, size: int) -> Block:
        """ Returns a randomly generated child block with all required block
        attributes given the initial <level> of the block, <maxi> or maximum
        depth, and the <size> of the board.

        This helper is recursively called within itself to subdivide children
        based on the random number.

        Precondition:
        level <= maxi
        """
        if random.random() <= math.exp(-0.25 * level) and level < maxi:
            new_block = Block(self.position, size, None, level, maxi)
            for _ in range(4):
                new_block.children.append(self._smash_helper(
                    level + 1, maxi, new_block._child_size()))
        else:
            new_block = Block(self.position, size,
                              random.choice(COLOUR_LIST), level, maxi)
        return new_block
    ...
    def paint(self, colour: Tuple[int, int, int]) -> bool:
        ...
        """
        if self.level == self.max_depth and self.colour != colour and not\
                self.children:
            self.colour = colour
            return True
        return False
    ...
    def combine(self) -> bool:
        ...
        if self.children and self.level == self.max_depth - 1:
            for child in self.children:
                if child.colour not in temp_list_colour:
                    temp_list_colour.append(child.colour)
                else:
                    duplicate_list.append(child.colour)
            if len(duplicate_list) == 0:
                return False
            elif len(duplicate_list) == 1:
                majority = duplicate_list[0]
            elif len(duplicate_list) == 2:
                if set(duplicate_list) == set(temp_list_colour):
                    return False
                else:
                    majority = duplicate_list[0]
            elif len(duplicate_list) == 3:
                majority = temp_list_colour[0]
