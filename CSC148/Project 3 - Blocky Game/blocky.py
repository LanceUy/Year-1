...
def _block_to_squares(board: Block) -> List[Tuple[Tuple[int, int, int],
    ...
    if not board.children:
        ...
        for i in range(len(board.children)):
            lst_of_all += _block_to_squares(board.children[i])
    return lst_of_all
...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Process the <event> from the operating system for players,
        if possible.
        """
        self._current_player().process_event(event)

    def update(self) -> GameState:
        """Update the player GameState based on past events.
    ...
    def render(self, renderer: Renderer) -> None:
        """Render the current state of the game onto the screen with <renderer>.
        """
...
class AnimateMoveState(GameState):
    ...
    def update(self) -> GameState:
        """Update the animation GameState based on past events.
    ...
    def render(self, renderer: Renderer) -> None:
        """Render the player move animation before passing it to GameState with
        <renderer>.
        """
...
class GameOverState(GameState):
    ...
    def update(self) -> GameState:
        """Update the game over GameState based on past events.
    ...
    def render(self, renderer: Renderer) -> None:
        """Render the game over screen before passing it to GameState with
        <renderer>.
        """
        x = 10
        y = 10
