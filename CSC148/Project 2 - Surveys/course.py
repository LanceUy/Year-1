...
class Student:
    ...
    === Private Attributes ===
    _qna: dictionary containing
    the question and answer of the
    student
    ...
    def has_answer(self, question: Question) -> bool:
        ...
        """
        if question in self._qna:
            return self._qna[question].is_valid(question)
        else:
            return False
        ...
        for new_student in students:
            for student in self.students:
                if new_student.id == student.id:
                    violates = True
            for new_student2 in students:
                if new_student2.id == new_student.id and \
                        id(new_student) != id(new_student2):
                    violates = True
        if not violates:
